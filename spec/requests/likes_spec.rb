...
describe 'Likes API', type: :request do
  ...
  context "GET /likes for user" do
    it "should return an array of likes for a given user with the correct auth" do
      get "/api/v1/users/#{user.id}/likes", headers: {"Authorization" => "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VybmFtZSI6Ik1lIn0.se5D8jyHoEwVnR-t2qivsTzTXFW5QsMgncTrAoenSYM" }
      expect(response).to have_http_status(:success)
      expect(response_body["likes"].size).to eq(Like.all.size)
      expect(response_body).to eq({"meta"=>
        {"total_likes"=>2},
        "likes"=>
         [
           {
  ...
  context "POST delete like" do
    it "It should return a ok and message when a like is removed" do
      delete "/api/v1/records/#{record.id}/likes/#{like.id}", headers: {"Authorization" => "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VybmFtZSI6Ik1lIn0.se5D8jyHoEwVnR-t2qivsTzTXFW5QsMgncTrAoenSYM" }
